...
int main(int argc, char **argv){
	...
	while((option=getopt(argc,argv,"n:m:p:b:tawsdg"))!=-1){
		switch(option){
			...
			case 's': serial = 1;
				break;
			case 'd': mallocPitch = 0; // enable or disable use of mallocPitch
				break;
			case 'g': glob = 1;		// turn on or off unoptimised kernal //
				break;
			default:
	...
	if(block_size>2048){
		printf("Block size too large. Not enough space on shared memory\n");
		return 1;
	}
    ///////////////////////// SETUP //////////////////////////////////////
	
	// allocating all the memory on RAM //
	...
	// copying to grids for use on GPU //
